...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        if score < 0 or score > 100:
            return Response({"res": "Score must be between 0 - 100."}, status=status.HTTP_400_BAD_REQUEST)

        # # Filter Objects Example
        # DataModel.objects.filter(filed_1=value_1, filed_2=value_2, filed_2=value_3)
        query_set = Personnel.objects.filter(first_name=student_first_name, last_name=student_last_name)

        if len(query_set) == 0:
            return Response({"res": "Student does not exists"}, status=status.HTTP_400_BAD_REQUEST)

        person = query_set[0]

        if person.personnel_type != 2:
            return Response({"res": "This Person is not Student"}, status=status.HTTP_400_BAD_REQUEST)
        
        student_id = person.pk
        
        subject_id=0
        for dict_subject in subjects_context:
            if subjects_title == dict_subject["title"]:
                subject_id = dict_subject["id"]
                break
        if subject_id == 0: return Response({"res": "There is no Subject :"+ subjects_title}, status=status.HTTP_400_BAD_REQUEST)

        is_exist_student_subjects_score = StudentSubjectsScore.objects.filter(student_id=student_id,).filter(subjects_id=subject_id).exists()
        print(is_exist_student_subjects_score)

        if is_exist_student_subjects_score:
            res = "Updated StudentSubjectsScore"
            print("to update")
            StudentSubjectsScore.objects.filter(student_id=student_id, subjects_id=subject_id).update(score=score)
        else:
            res = "Created StudentSubjectsScore"
            for _credits_mapping in credits_mapping:
                if _credits_mapping["subject_id"] == subject_id:
                    credit_context_id = _credits_mapping["credit_id"]
                    for _credit in credits_context:
                        if _credit["id"] == credit_context_id:
                            credit = _credit["credit"]
                            break
            # # Create Objects Example
            # DataModel.objects.create(filed_1=value_1, filed_2=value_2, filed_2=value_3)
            print("to create")
            StudentSubjectsScore.objects.create(student_id=student_id, subjects_id=subject_id, score=score, credit=credit)

        return Response({"res": res},status=status.HTTP_201_CREATED)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        if student.personnel_type != 2:
            return Response({"res": "This Person is not Student"}, status=status.HTTP_400_BAD_REQUEST)
        
        if student is None:
            return Response({"res": "Student with id" + student_id + " does not exists"}, status=status.HTTP_400_BAD_REQUEST)
        
        student_subject_scores = StudentSubjectsScore.objects.filter(student_id=student.pk)
        list_subject_detail = []
        sum_score = 0
        for student_subject_score in student_subject_scores:
            ...
            list_subject_detail.append(subject_detail)
            sum_score += score
        
        avg_score = sum_score / len(list_subject_detail)
        ...
        context_data = {
            ...
            "subject_detail": list_subject_detail,
            "grade_point_average": get_grade(avg_score),
        }
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        _school_id = Schools.objects.get(title=school_title).pk

        list_classes = Classes.objects.filter(school_id=_school_id)

        dict_personal_type_role = { 
            0: "Teacher", 1: "Head of the room", 2:"Student"
        }
        order = 1
        your_result = []
        for personal_type,role in dict_personal_type_role.items():
            for _class in list_classes:
                list_person_object_classorder = Personnel.objects.filter(personnel_type=personal_type,school_class_id=_class.pk)
                list_person_name = []
                for person in list_person_object_classorder:
                    list_person_name.append(person.first_name +" "+ person.last_name)
                list_person_name.sort()
                for person_name in list_person_name:
                    class_order = _class.class_order
                    result_data = f"{order}. school: {school_title}, role: {role}, class: {class_order}, name: {person_name}."
                    your_result.append(result_data)
                    order += 1
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        query_object_schools =  Schools.objects.all()
        for object_school in query_object_schools:
            dict_school = {}
            dict_school[object_school.title] = {}

            query_object_classes = Classes.objects.filter(school=object_school)
            for object_classes in query_object_classes:
                class_key = "class"+str(object_classes.class_order)
                dict_school[object_school.title][class_key] = {}

                query_obj_teacher = Personnel.objects.get(school_class=object_classes,personnel_type=0)
                teacher_name = query_obj_teacher.first_name + " " + query_obj_teacher.last_name
                title = dict_personal_type_role[0]
                key_teacher = title+": "+ teacher_name
                dict_school[object_school.title][class_key][key_teacher] = []
                
                query_obj_class_head = Personnel.objects.get(school_class=object_classes,personnel_type=1)
                class_head_name = query_obj_class_head.first_name + " " + query_obj_class_head.last_name
                title = dict_personal_type_role[1]
                class_head = {}
                class_head[title] = class_head_name
                dict_school[object_school.title][class_key][key_teacher].append(class_head)

                query_obj_class_student = Personnel.objects.filter(school_class=object_classes,personnel_type=2)
                title = dict_personal_type_role[2]
                for obj_student in query_obj_class_student:
                    class_student_name = obj_student.first_name + " " + obj_student.last_name
                    class_student = {}
                    class_student[title] = class_student_name
                    dict_school[object_school.title][class_key][key_teacher].append(class_student)

            your_result.append(dict_school)
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        your_result = []
        dict_level_struct_id = {}
        dict_level_struct_id["total_level"] = 1
        # dict_level_struct_id["level_list"] = []

        query_set_obj_lvl0 = SchoolStructure.objects.filter(parent=None)
        list_sub = []
        dict_parent_sub = {}
        dict_sub_parent = {}
        dict_level_struct_id[0] = []
        dict_parent_sub[0] = []
        for obj_lvl0 in query_set_obj_lvl0:
            dict_level_struct_id[0].append(obj_lvl0.pk)
            dict_parent_sub[0].append(obj_lvl0.pk)
            dict_sub_parent[obj_lvl0.pk] = 0
            # dict_level_struct_id[0] = obj_lvl0.pk

        i = 1
        is_end = False
        while(is_end is False):
            list_parent_id = dict_level_struct_id[i-1]
            dict_level_struct_id[i] = []
            for parent_id in list_parent_id:
                dict_parent_sub[parent_id] = []
                query_set_obj_lvln = SchoolStructure.objects.filter(parent_id=parent_id)
                if len(query_set_obj_lvln) == 0:
                        is_end = True
                for obj_lvln in query_set_obj_lvln:
                    # dict_lvln[i].append(obj_lvln.pk)
                    # dict_parent_sub[parent_id] = obj_lvln.pk
                    dict_level_struct_id[i].append(obj_lvln.pk)
                    dict_parent_sub[parent_id].append(obj_lvln.pk)
                    dict_sub_parent[obj_lvln.pk] = parent_id
                    # dict_level_struct_id[i].append({parent_id:obj_lvln.pk})
            i += 1
        dict_level_struct_id["total_level"] = i-1

        # return Response(dict_level_struct_id, status=status.HTTP_200_OK)

        list_id = dict_parent_sub[0]
        
        
        # for id in list_id:
        #     # your_result.append({"id":id, "sub": []})
        #     has_sub = True
        #     while has_sub:
        #         if len(dict_parent_sub[id]) == 0:
                    
                # else:
                    # dict_sub = {}
                    # dict_sub["id"] = id
            # your_result.append({"id":})




        # for i in range(dict_level_struct_id["total_level"]):
        # for root_id in dict_level_struct_id[0]:
        #     dict_parent = {}
        #     dict_parent["title"] = SchoolStructure.objects.get(pk=root_id).title
        #     dict_parent["sub"] = []
        #     num_sub = dict_parent_sub[root_id]
        #     while(num_sub > 0):
        #         dict(sub)
        #         dict_parent["sub"].append()
            # for i in range(1,dict_level_struct_id["total_level"]):
                # dict_parent_sub[root_id]
            #     query_set_obj_sub = SchoolStructure.objects.filter(parent_id=parent_id)
            #     for obj_sub in query_set_obj_sub:
            #         dict_sub = {}
            #         dict_sub["title"] = obj_sub.title
            #         dict_parent_sub["sub"].append(dict_sub)
            # your_result.append(dict_parent)

        return Response(dict_parent_sub, status=status.HTTP_200_OK)
        return Response(your_result, status=status.HTTP_200_OK)